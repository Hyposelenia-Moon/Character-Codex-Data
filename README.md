...
写成 `暴击率 / 暴击伤害` → 显示 `暴击率 = 暴击伤害`；**其余相邻词条一律是优先级**，写成 `>` → 显示 `＞`：

```text
文档：副词条：暴击率 / 暴击伤害 > 大攻击 > 元素精通
显示：副词条：暴击率 = 暴击伤害 ＞ 大攻击 ＞ 元素精通
```

**不要在非暴击对之间写 `/`**（那会被当成"同级"、渲染成 `=`）。显示层有兜底
（`subSepBetween`：只有暴击对才把 `/` 显示成 `=`，其余一律 `＞`），`scan-separators.mjs`
也会把非法同级对当错误拦下来。
