...
- **PR 正文 = 各提交 `- ` 要点的拼接**：按提交顺序（旧→新），重复的合并成一条，**不带提交标题**
  （标题只用于提交本身与 PR 标题）
- 自查：`git log --reverse --format=%B <base>..HEAD`，抽其中的 `- ` 行即为正文
- 没有 `- ` 要点说明提交信息不完整，先 `git commit --amend` 补齐

书写要求：

1. **书面语**：不使用口语与网络语，不使用情绪化表达
   （避免「弄 / 搞 / 炸 / 跑一遍 / 顺手 / 白做 / 就是这么」这类写法）
2. **不用 emoji、不用感叹号**，句末不加标点
3. **不用第一人称**（我 / 咱们 / 我们），主语是改动本身
4. 标题一句话说清"改了什么"，不超过 50 字；正文每条只写一个改动点，写"改了什么 + 为什么或影响"
5. 术语用仓库既有词：主文档 / 标记版 / 网页版（`guide.html`）/ 文档版（`guide.md`）/
   编辑器 / 面板 / 数据仓库 / 攻略页面
6. 文件路径、函数名、提交号、尺寸与字号等标识用反引号包裹，照抄原文
7. 类型前缀沿用仓库既有风格：`feat` / `fix` / `docs` / `test` / `refactor` / `chore`，
   或本仓库专用范围（`data` / `editor` / `display` / `docx` / `web`），例：`data: 归一主词条括注写法`
...
`>`（显示 `＞`）**优先级/顺序**；`≥` 保留原样。`｜` 只用于主词条三个**槽位**之间的并列。

**副词条的特例（用户定稿，最容易搞错的一条）**：副词条里**只有「暴击率 ↔ 暴击伤害」是同级**，
其余相邻词条一律是优先级。**显示规则与源文档写什么无关**（写 `/` 还是 `>` 都一样）：

```text
文档：副词条：暴击率 / 暴击伤害 > 大攻击 > 元素精通     文档：副词条：元素精通 > 暴击率 > 暴击伤害
显示：副词条：暴击率 = 暴击伤害 ＞ 大攻击 ＞ 元素精通   显示：副词条：元素精通 ＞ 暴击率 = 暴击伤害
```

- 暴击对**恒为 `=`**（`≥` 是显式写法，原样保留）；
- 其余相邻词条**恒为 `＞`** —— 就算源文档写了同级 `/`，也照样显示 `＞`；
- 判据是**显示文本**（`暴击` / `暴伤` 已展开成 `暴击率` / `暴击伤害`），由 `subSepBetween` 实现。

**不要在非暴击对之间写 `/`**（显示层会兜底成 `＞`，但文档读起来会误导）：
`scan-separators.mjs` 会把非法同级对当错误拦下来。

**副词条逐档分隔符**：`v2.artifacts[kind=sub].sep` 按**空白分隔的逐档 token** 存
（`' / > > '` ＝ 第 1 档 `/`、第 2/3 档 `>`）；`parse-docx` 的 `gapSepOf` 生成、
`schema.renderArtifactRow` 的 `sepTokens` 逐字写回。所以一行里可以混用 `/` 与 `>`。

**圣遗物套装的分隔符（用户定稿，2026-09-20 二次修订）**：源文档里的 `/` 是**同级**（"这套或那套都行"）→
显示成**字面 `/`**（`教官/勇者`、`翠绿之影/2攻击`）；`>` / `≥`（优先级）才画 `＞`。
`+` **只有两侧都是件数简写（`2X`/`4X`）时**才算「同一组合」（2+2 才凑得满 4 件）→ 留在**同一个 chip 内**；
**全套装名之间的 `+` 其实是同级选项 → 显示 `/`**（用户原话：「同级圣遗物套装之间依旧有部分角色为 `+` 而不是 `/`」）。
见 `guide-display.mjs` 的 `SET_LEVEL_SEP` / `gapSepOf` / `isPieceShorthand` / `resolveSetItems`：

```text
文档：首选：教官 / 烬城勇者绘卷        →  显示：[教官]/[烬城勇者绘卷]     （同级：字面 `/`）
文档：首选：如雷的盛怒 + 昔日宗室之仪   →  显示：[如雷的盛怒]/[昔日宗室之仪]（全名之间：同级）
文档：过渡：2精通 + 2精通             →  显示：[2精通+2精通]           （2+2：同一 chip）
文档：首选：2生命 + 2充能 + 角斗士…    →  显示：[2生命+2充能]/[角斗士…] （简写组合 vs 同级）
...
```text
文档：满拐：岩峰巡歌 / 圣显之钥        →  显示：[岩峰巡歌]＞[圣显之钥]
```

「**同级且毫无区别**」不写在分隔符上，而是写在**条目文字本身** —— 例如莉奈娅的
`88爆伤/44暴击武器` 是**一个条目名**（两把武器同级、没区别），名字里的 `/` 原样保留、不会被当分隔符。
⚠ 注意名字里的 `/`、`｜` 在写回文档后再解析会被切成两条（往返不一致）——编辑器会打 ⚠、`scan-separators` 也会列出来。
真要用这种写法，正确姿势是**拆成两条、用 `/` 相连**（`88爆伤 / 44暴击武器`）：显示一模一样，且能往返。
...
**例外 · 天赋**（用户定稿 2026-09-24）：天赋模块**不显示「暂无」** —— 没填 / 三格全 1 一律按
`A 1 ／ E 1 ／ Q 1` 正常画三格（网页版 `build-html.mjs` 的 `defaultTalentRow`、面板 `parse.js` 的
`v2TalentRows` 各兜一层；正常数据里 `build-docx` 会给每个角色补上 `天赋：A1 E1 Q1` 行，
所以 `guide.md` 与主文档也有这一行）。

### 攻略图（游戏内面板）的呈现约定

面板与网页版共用同一份显示归一（`guide-display.mjs` ↔ 插件 `display.js` 逐字节一致），
但**画法**（模板 + CSS）各自实现。用户逐条定稿的呈现规则：
...
### 主词条括注：**只有「写在值里」一种形状**（别用 `note` / `noteSlot` 字段）

主词条的补充说明跟在**那个部位的某个值末尾**，写全角括弧：

```json
{ "kind": "main", "stats": { "时之沙": ["攻击力", "元素精通（高命）"], "空之杯": ["攻击力"], "理之冠": ["暴击伤害", "暴击率"] } }
```

`parse-docx` 按用户定稿（2026-09-20）**不**把值末尾的括注剥成 `note` / `noteSlot` 字段，
所以文档层只有这一种形状。编辑器表单里的「主词条括注」输入框 + 「挂在哪个部位」下拉是**输入便利**：
保存时由 `foldMainNoteIntoStats()` 折进 `noteSlot` 那个部位的**最后一个值**上（值是 `元素精通（高命）`），
**不落** `note` / `noteSlot` 字段。

> 反例（曾经导致「保存并发布」失败，用户 2026-09-26 报过）：数据里留
> `{"kind":"main","note":"高命","noteSlot":"时之沙", …}` —— 写进 docx 是 `时之沙：攻击力 / 元素精通（高命）`，
> 读回来括注已经在值里、`note` 字段回不来 ⇒ 往返校验 128/129 ⇒ 发布被挡下。
> `scripts/scan-separators.mjs` 现在把这种写法当**错误**拦下来（退出码 1）。
...
- **切到后台的标签页不算"人走了"**：心跳在 `visibilityState === 'hidden'` 时**照发**（带 `{hidden:true}`），
  服务端把阈值放宽到 180s（浏览器会给后台定时器降频到 ≈1 次/分钟，用 20s 阈值会把还开着的页面误杀 ——
  表现就是切回来「预览失败 / 保存失败：Failed to fetch」）。真正关窗口仍由 `/api/close` 在 5 秒后收掉
...
**提交摘要**（落盘 `out/_commit-summary.md`；界面上默认**不摊开**，点成功 toast 的「复制提交信息」按钮即可复制全文）包含四段：
...
node scripts/fix-docx-stats.mjs --dry      # 主词条去「百分比」+ 副词条固定值写 小X + 暴击对用 `/`、其余 `/` 改 `>` + 2+2 属性词简写、去件数
```
