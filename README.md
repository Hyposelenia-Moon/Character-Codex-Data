...
提交信息（也是 PR 正文的来源）就一个格式：

```
标题（fix: / feat: / docs: … + 中文简述）

- 要点（一条一行，尽量短；只写改了什么）
```

- **PR 正文 = 各提交的要点，纯正文、不带标题**（标题只用于提交本身和 PR 标题）
- 多提交按提交顺序（旧→新）拼接，重复的要点合并成一条
- 自查：`git log --reverse --format=%B upstream/master..HEAD`，把里面的 `- ` 行抽出来就是正文
