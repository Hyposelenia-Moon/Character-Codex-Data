...
**圣遗物套装的分隔符（用户定稿）**：源文档里的 `/` 是**同级**（"这套或那套都行"）→
显示时**两个 chip 紧挨着、不画 `＞`**；`+` 是**同一组合**（2+2）→ 仍在**同一个 chip 内**用 `+` 连接；
只有 `>` / `≥`（优先级）才画 `＞`。见 `guide-display.mjs` 的 `SET_LEVEL_SEP` / `gapSepOf`：

```text
文档：首选：翠绿之影 / 2攻击          →  显示：[翠绿之影][2攻击]        （同级：紧挨着）
