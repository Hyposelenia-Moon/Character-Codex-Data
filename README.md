...
**圣遗物套装的分隔符（用户定稿，2026-09-20 修订）**：源文档里的 `/` 是**同级**（"这套或那套都行"）→
显示成**字面 `/`**（`教官/勇者`、`翠绿之影/2攻击`）；`+` 是**同一组合**（2+2）→ 仍在**同一个 chip 内**用 `+` 连接；
`>` / `≥`（优先级）才画 `＞`。见 `guide-display.mjs` 的 `SET_LEVEL_SEP` / `gapSepOf` / `displaySep`：
...
| 位置 | 规则 | 实现 |
|---|---|---|
| 圣遗物 | **同级套装（源文档 `/`）画字面 `/`**（`教官/勇者`）；2+2 仍在一个 chip 内用 `+` | `guide-display.mjs` 的 `SET_LEVEL_SEP`/`gapSepOf`/`displaySep` ＋ `parse.js` 的 `resolveArtifactSetItems` |
