/**
 * 一次性生成 LICENSE-DATA（CC BY 4.0 官方英文 legalcode 全文 + 中文适用范围头）
 *
...
const OFFICIAL = 'https://creativecommons.org/licenses/by/4.0/legalcode.txt'

/** 中文适用范围头（照上一版结构，只改许可名与要素） */
const HEAD = `Character-Codex-Data — 数据与文档部分许可
...
guide.html、guide.md——采用「知识共享 署名 4.0 国际」
（Creative Commons Attribution 4.0 International，简称 CC BY 4.0）许可协议授权。
...
许可要素摘要（仅供参考，不构成法律文本）：
  ...
  - 允许商用：可用于商业目的，无需另行取得授权。
  - 无相同方式共享（No SA）：改编作品不必以相同许可发布（但仍须署名）。

注意：
  1. 本文件正文为 Creative Commons 官方发布的 CC BY 4.0 英文法律文本，逐字未改动。
     官方纯文本：https://creativecommons.org/licenses/by/4.0/legalcode.txt
     官方页面：  https://creativecommons.org/licenses/by/4.0/legalcode
     简体中文参考译本：https://creativecommons.org/licenses/by/4.0/legalcode.zh-Hans
     若本文件与官方文本存在任何出入，一律以官方文本为准。
  2. 本仓库的代码部分（scripts/、resources/）采用 MIT 许可，见 LICENSE 文件。
...
const content = HEAD + official.trimEnd() + '\n'
...
if (checkOnly) {
  ...
  const same = diffAt === -1
  console.log(`官方英文正文：${official.length} 字节，sha1=${crypto.createHash('sha1').update(official).digest('hex').slice(0, 12)}`)
